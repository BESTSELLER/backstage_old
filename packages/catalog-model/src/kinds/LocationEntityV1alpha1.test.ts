...
describe('LocationV1alpha1Validator', () => {
  ...
  it('accepts good target', async () => {
    (entity as any).spec.target =
      'https://github.com/BESTSELLER/backstage/blob/master/plugins/catalog-backend/examples/artist-lookup-component.yaml';
    await expect(validator.check(entity)).resolves.toBe(true);
  });
  ...
  it('accepts good targets', async () => {
    (entity as any).spec.targets = [
      'https://github.com/BESTSELLER/backstage/blob/master/plugins/catalog-backend/examples/artist-lookup-component.yaml',
      'https://github.com/BESTSELLER/backstage/blob/master/plugins/catalog-backend/examples/playback-order-component.yaml',
    ];
    await expect(validator.check(entity)).resolves.toBe(true);
  });
